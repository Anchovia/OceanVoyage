...
float sampleShadowCSM(vec3 worldPos, float viewDepth, vec3 normal, vec3 lightDir) {
    ...
    if (viewDepth > ubo.cascadeSplits.z) return 1.0; // beyond the last cascade → lit
    ...
    float bias  = mix(0.0015, 0.0003, NdotL) * float(cascade + 1); // bigger cascades need more bias
