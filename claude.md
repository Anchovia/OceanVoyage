...
**Scope — applies to code architecture, NOT rendering quality.** For visual/rendering work, default to the proper high-quality technique within the RTX 3060-class budget (FFT/Gerstner ocean, screen-space reflections, high-resolution / CSM shadows, PBR ship materials, mipmaps + anisotropic filtering, authored textures, soft shadows). Do NOT cheapen visuals or remove/fake features to be "simple" or "low-spec" — the minimal-looking code (64² textures, flat shading, 2048 shadow map) is placeholder, and the target is high-quality realistic ocean (README 목표 / DESIGN: 사실적 해양 north star). If a cheaper visual path is genuinely better, say so explicitly and let the user decide — never pick it silently.
...
4. Check for conflicts with `CLAUDE.md`, `ARCHITECTURE.md`, `README.md` and `DESIGN.md`
